...
class ExtrinsicCalibrator:public Vrui::Application
	...
	/* Elements: */
	Threads::EventDispatcherThread dispatcher; // Event dispatcher
...
void ExtrinsicCalibrator::saveAlignmentCallback(Misc::CallbackData* cbData)
	...
	IO::OStream extrinsicFile(IO::openFile(extrinsicFileName.c_str(),IO::File::WriteOnly));
...
ExtrinsicCalibrator::ExtrinsicCalibrator(int& argc,char**& argv)
	...
	if(remoteSource!=0)
		...
		{
		Kinect::DirectFrameSource* directCamera=Kinect::openDirectFrameSource(cameraIndex,true);
		camera=directCamera;
...
ExtrinsicCalibrator::~ExtrinsicCalibrator(void)
	...
	{
	IO::FilePtr tiePointFile=IO::openFile("FullCalibTiePoints.dat",IO::File::WriteOnly);
	...
	for(std::vector<FullCalibTiePoint>::iterator tpIt=fullCalibTiePoints.begin();tpIt!=fullCalibTiePoints.end();++tpIt)
		...
		tiePointFile->write(tpIt->first.getTranslation().getComponents(),3);
		tiePointFile->write(tpIt->first.getRotation().getQuaternion(),4);
		
		/* Write the disk position: */
		tiePointFile->write(tpIt->second.getComponents(),3);
		}
	}
