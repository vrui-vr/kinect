...
Camera::StreamingState::StreamingState(libusb_device_handle* handle,unsigned int endpoint,Camera* sCamera,int sPacketFlagBase,int sPacketSize,const Size& sFrameSize,size_t sRawFrameSize,Camera::StreamingCallback* sStreamingCallback)
	:camera(sCamera),
	 ...
	 frameSize(sFrameSize),rawFrameSize(sRawFrameSize),rawFrameBuffer(new unsigned char[rawFrameSize*2]),
...
size_t Camera::sendMessage(Camera::USBWord messageType,const Camera::USBWord* messageData,size_t messageSize,void* replyBuffer,size_t replyBufferSize)
	...
	if(messageSize>252)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Message too long");
	...
	/* Check if the message buffer is large enough to hold the device's response: */
	if(replyBufferSize>=sizeof(messageBuffer))
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Expected response too long");
	...
	if(replySize<4*sizeof(USBWord)||messageBuffer[0]!=0x4252U||messageBuffer[2]!=messageType||messageBuffer[3]!=messageSequenceNumber-1)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Protocol error while sending message %u",(unsigned int)messageType);
	
	/* Copy the message reply to the provided buffer: */
	memcpy(replyBuffer,messageBuffer,Math::min(replySize,replyBufferSize));
...
bool Camera::sendCommand(Camera::USBWord command,Camera::USBWord value)
	...
	USBWord replyBuffer[5];
	size_t replySize=sendMessage(0x0003U,commandBuffer,2,replyBuffer,sizeof(replyBuffer));
	
	/* Check for success message: */
	return replySize==sizeof(replyBuffer)&&replyBuffer[1]==1&&replyBuffer[4]==0x0000U;
	}
...
Camera::USBWord Camera::readRegister(Camera::USBWord address)
	...
	USBWord replyBuffer[7];
	size_t replySize=sendMessage(0x0095U,commandBuffer,3,replyBuffer,sizeof(replyBuffer));
	
	/* Check for success: */
	if(replySize!=sizeof(replyBuffer)||replyBuffer[4]!=0x0000U)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Protocol error");
	
	/* Return the read register value: */
	return replyBuffer[6];
	}
...
void* Camera::colorDecodingThreadMethod(void)
	...
	while(true)
		...
		unsigned int width=streamers[COLOR]->frameSize[0];
		unsigned int height=streamers[COLOR]->frameSize[1];
		FrameBuffer decodedFrame(streamers[COLOR]->frameSize,height*width*sizeof(ColorPixel));
...
void* Camera::depthDecodingThreadMethod(void)
	...
	while(true)
		...
		unsigned int width=streamers[DEPTH]->frameSize[0];
		unsigned int height=streamers[DEPTH]->frameSize[1];
		FrameBuffer decodedFrame(streamers[DEPTH]->frameSize,height*width*sizeof(DepthPixel));
		decodedFrame.timeStamp=frameTimeStamp;
		...
		/* Process rows: */
		for(unsigned int y=0;y<height;++y,dRowPtr-=width) // Flip the depth image vertically
			{
			DepthPixel* dPtr=dRowPtr;
			
			/* Process pixels in groups of eight: */
			for(unsigned int x=0;x<width;x+=8,sPtr+=11,dPtr+=8)
				{
...
void* Camera::compressedDepthDecodingThreadMethod(void)
	...
	while(true)
		...
		for(unsigned int y=0;y<height;++y,dRowPtr-=width) // Flip the depth image vertically
			...
			unsigned int lastPixel=invalidDepth;
			while(dPtr!=dEnd)
				{
...
void Camera::colorExposureSliderCallback(GLMotif::TextFieldSlider::ValueChangedCallbackData* cbData)
	{
	/* Print current exposure: */
	Misc::formattedConsoleNote("Exposure: %u",(unsigned int)readRegister(0x0009U));
	
	/* Set color exposure value: */
	setExposure(int(Math::floor(cbData->value+0.5)));
	}
...
Camera::Camera(libusb_device* sDevice)
	:device(sDevice),
	 ...
	 compressDepthFrames(true),smoothDepthFrames(true),irIntensity(30U),nearMode(false),exposure(512),sharpening(0)
	 #if KINECT_CAMERA_DUMP_HEADERS
	 ,headerFile(0)
	 #endif
	{
...
Camera::Camera(size_t index)
	...
	if(!device.isValid())
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Fewer than %d Kinect camera devices detected",int(index)+1);
...
Camera::Camera(const char* serialNumber)
	...
	if(dev==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Kinect device with serial number %s not found",serialNumber);
...
void Camera::startStreaming(FrameSource::StreamingCallback* newColorStreamingCallback,FrameSource::StreamingCallback* newDepthStreamingCallback)
	...
	***********************************/
	...
	if(!sequenceOk)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Failed to disable cameras");
	...
	**********************************************************/
	...
	if(streamers[COLOR]!=0)
		...
		#if 0 // Experiment with some unknown registers that seem to affect auto-exposure
		/* Dump some register values: */
		for(USBWord reg=0;reg<0x0010U;++reg)
			Misc::formattedConsoleNote("Register 0x%04x = 0x%04x",(unsigned int)reg,(unsigned int)readRegister(reg));
		
		// writeRegister(0x0008U,0x005fU); // Default value after reset
		writeRegister(0x0008U,0x005fU);
		
		// writeRegister(0x000aU,0x8011U); // Default value after reset
		writeRegister(0x000aU,0x8011U);
		#endif
		...
		if(sequenceOk)
			...
			try
				{
				/* Set the color camera's exposure value: */
				writeRegister(0x0009U,exposure);
...
void Camera::configure(Misc::ConfigurationFileSection& configFileSection)
	...
	/* Set color camera exposure and sharpening values: */
	setExposure(configFileSection.retrieveValue<unsigned int>("./colorExposure",getExposure()));
...
void Camera::buildSettingsDialog(GLMotif::RowColumn* settingsDialog)
	...
	new GLMotif::Label("ColorExposureLabel",sliderBox,"Color Exposure");
	
	GLMotif::TextFieldSlider* colorExposureSlider=new GLMotif::TextFieldSlider("ColorExposureSlider",sliderBox,4,ss.fontHeight*6.0f);
	colorExposureSlider->setSliderMapping(GLMotif::TextFieldSlider::LINEAR);
	colorExposureSlider->setValueType(GLMotif::TextFieldSlider::UINT);
	colorExposureSlider->setValueRange(0,1023,1);
	colorExposureSlider->setValue(exposure);
	colorExposureSlider->getValueChangedCallbacks().add(this,&Camera::colorExposureSliderCallback);
...
void Camera::getCalibrationParameters(Camera::CalibrationParameters& calib)
	...
	for(int subset=0;subset<4;++subset)
		...
		if(sendMessage(subset<3?0x0016U:0x0004U,cmdBuffer,subset<3?5:1,replyBuffer.getMemory(),calibrationParameterReplySizes[subset])!=calibrationParameterReplySizes[subset])
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Protocol error while requesting parameter subset");
...
unsigned int Camera::getExposure(void)
	{
	if(streamers[COLOR]!=0)
		{
		/* Query the exposure register value: */
		exposure=readRegister(0x0009U);
		}
	
	return exposure;
	}

void Camera::setExposure(unsigned int newExposure)
	{
	/* Limit the exposure value: */
	exposure=Math::clamp(newExposure,0U,1023U);
