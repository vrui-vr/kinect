...
Motor::Motor(size_t index)
	...
	if(!isValid())
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Fewer than %d Kinect motor devices detected",int(index)+1);
...
void Motor::readAccelerometers(float accels[3])
	...
	if(dataSize>=8)
		...
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Short control packet, received %d bytes instead of 8",int(dataSize));
	}
