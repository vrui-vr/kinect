...
void KinectV2JpegStreamReader::skipInputDataFunction(j_decompress_ptr cinfo,long count)
	...
	if(count<0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unable to skip backwards");
...
void* KinectV2JpegStreamReader::decompressionThreadMethod(void)
	...
	while(true)
		...
		/* Set the decompressor's output color space: */
		decompressor.out_color_space=forceRgb?JCS_RGB:JCS_YCbCr;
		...
		Size frameSize(decompressor.output_width,decompressor.output_height);
		FrameBuffer decompressedFrame(frameSize,frameSize.volume()*sizeof(FrameSource::ColorPixel));
		...
		*************************************************************/
		...
		if(imageHeight!=frameSize[1])
			{
			delete[] imageRowPointers;
			imageHeight=frameSize[1];
			imageRowPointers=new FrameSource::ColorPixel*[imageHeight];
			}
		FrameSource::ColorPixel* rowPtr=decompressedFrame.getData<FrameSource::ColorPixel>()+(imageHeight-1)*frameSize[0];
		for(unsigned int y=0;y<imageHeight;++y,rowPtr-=frameSize[0])
...
void KinectV2JpegStreamReader::setForceRgb(bool newForceRgb)
	{
