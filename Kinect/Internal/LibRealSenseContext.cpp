...
void LibRealSenseContext::ref(void)
	...
	if((refCount++)==0)
		...
		if(error!=0)
			{
			/* Throw an exception: */
			std::runtime_error exception=Misc::makeStdErr(__PRETTY_FUNCTION__,"Error %s while creating librealsense context",rs_get_error_message(error));
			rs_free_error(error);
			throw exception;
			}
		}
...
void LibRealSenseContext::unref(void)
	...
	if((--refCount)==0)
		...
		if(error!=0)
			{
			/* Throw an exception: */
			std::runtime_error exception=Misc::makeStdErr(__PRETTY_FUNCTION__,"Error %s while destroying librealsense context",rs_get_error_message(error));
			rs_free_error(error);
			throw exception;
			}
		context=0;
...
int LibRealSenseContext::getNumDevices(void) const
	...
	if(error!=0)
		{
		/* Throw an exception: */
		std::runtime_error exception=Misc::makeStdErr(__PRETTY_FUNCTION__,"Error %s while querying number of RealSense devices",rs_get_error_message(error));
		rs_free_error(error);
		throw exception;
		}
...
struct rs_device* LibRealSenseContext::getDevice(int index)
	...
	if(error!=0)
		{
		/* Throw an exception: */
		std::runtime_error exception=Misc::makeStdErr(__PRETTY_FUNCTION__,"Error %s while opening RealSense device %d",rs_get_error_message(error),index);
		rs_free_error(error);
		throw exception;
		}
