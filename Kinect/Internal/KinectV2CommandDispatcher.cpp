...
size_t KinectV2CommandDispatcher::execute(const KinectV2CommandDispatcher::Command& command,unsigned int numParameters)
	...
	if(sendSize!=commandSize)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Incomplete command transfer; %u of %u bytes",(unsigned int)sendSize,(unsigned int)commandSize);
	...
	if(command.maxResponseSize>0)
		...
		if(receiveSize!=sizeof(Reply)||reinterpret_cast<Reply*>(replyBuffer)->magic!=Reply::replyCompleteMagic)
			...
			if(reinterpret_cast<Reply*>(replyBuffer)->seq!=command.seq)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid command reply sequence number, %u instead of %u",reinterpret_cast<Reply*>(replyBuffer)->seq,command.seq);
	...
	if(receiveSize!=sizeof(Reply))
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid command reply size of %u bytes",(unsigned int)receiveSize);
	if(reply.magic!=Reply::replyCompleteMagic)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid command reply magic value %u",reply.magic);
	if(reply.seq!=command.seq)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid command reply sequence number, %u instead of %u",reply.seq,command.seq);
