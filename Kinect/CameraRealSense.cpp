...
void CameraRealSense::initialize(void)
	...
	frameSizes[0]=Size(640,480); // Size(1920,1080);
	frameSizes[1]=Size(640,480);
	frameRates[0]=30;
	frameRates[1]=30;
...
void CameraRealSense::setFrameSize(int camera,const Size& newFrameSize)
	{
	if(frameSizes[camera]!=newFrameSize)
		{
...
void CameraRealSense::setZRange(RSDepthPixel zMin,RSDepthPixel zMax)
	...
	if(zMin>=zMax)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid Z value range [%u, %u]",(unsigned int)zMin,(unsigned int)zMax);
