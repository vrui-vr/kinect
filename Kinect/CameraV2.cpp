...
CameraV2::CameraV2(size_t index)
	...
	if(!device.isValid())
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Fewer than %d Kinect v2 camera devices detected",int(index)+1);
...
CameraV2::CameraV2(const char* serialNumber)
	...
	if(dev==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"No Kinect v2 camera device with serial number %s found",serialNumber);
...
FrameSource::IntrinsicParameters CameraV2::getIntrinsicParameters(void)
	...
	if(IO::Directory::getCurrent()->getPathType(intrinsicParameterFileName.c_str())==Misc::PATHTYPE_FILE)
		...
		IntrinsicParameters::PTransform::Matrix& dum=result.depthProjection.getMatrix();
		dum=IntrinsicParameters::PTransform::Matrix::zero;
		dum(0,0)=1.0/dcp.sx;
		dum(0,3)=-(512.0-dcp.cx)/dcp.sx;
		dum(1,1)=1.0/dcp.sy;
		...
		result.depthProjection.leftMultiply(IntrinsicParameters::PTransform::scale(0.1));
...
const Size& CameraV2::getActualFrameSize(int sensor) const
	{
