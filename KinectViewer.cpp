...
KinectViewer::KinectStreamer::~KinectStreamer(void)
	{
	/* Shut down an active sphere extractor: */
	{
	Threads::Mutex::Lock sphereExtractorLock(sphereExtractorMutex);
	if(sphereExtractor!=0)
	...
	/* Shut down an active frame saver: */
	delete frameSaver;
	
	/* Delete the frame projector: */
	delete projector;
...
void KinectViewer::KinectStreamer::setTriangleDepthRange(int newTriangleDepthRange)
	{
	projector->setTriangleDepthRange(Kinect::FrameSource::DepthPixel(newTriangleDepthRange));
	}
...
void KinectViewer::startSavingStreams(const char* saveFileNameBase)
	{
	...
	std::string soundFileName=saveFileNameBase;
	...
	for(size_t i=0;i<streamers.size();++i)
		...
		std::string colorFileName=saveFileNameBase;
		...
		std::string depthFileName=saveFileNameBase;
...
void KinectViewer::saveStreamsOKCallback(GLMotif::FileSelectionDialog::OKCallbackData* cbData)
	...
	startSavingStreams(cbData->getSelectedPath().c_str());
...
KinectViewer::KinectViewer(int& argc,char**& argv)
	...
	for(int i=1;i<argc;++i)
		...
		if(argv[i][0]=='-')
			...
			if(strcasecmp(argv[i]+1,"h")==0)
				...
				compressDepth=false;
			else if(strcasecmp(argv[i]+1,"tdr")==0)
				{
				++i;
				triangleDepthRange=atoi(argv[i]);
				}
			else if(strcasecmp(argv[i]+1,"save")==0)
				{
				/* Save 3D video from all streamers: */
				++i;
				saveFileName=argv[i];
				}
				...
				depthFileName.append(".depth");
				Kinect::FileFrameSource* fileSource=new Kinect::FileFrameSource(IO::openFile(colorFileName.c_str()),IO::openFile(depthFileName.c_str()));
				...
				Kinect::MultiplexedFrameSource* source=Kinect::MultiplexedFrameSource::create(Comm::openTCPPipe(argv[i-1],atoi(argv[i])));
				
				/* Add a new streamer for each component stream in the multiplexer: */
				for(unsigned int i=0;i<source->getNumStreams();++i)
					{
					KinectStreamer* streamer=new KinectStreamer(this,source->getStream(i));
					if(triangleDepthRange>=0)
						streamer->setTriangleDepthRange(triangleDepthRange);
					streamers.push_back(streamer);
					}
	...
	if(printHelp)
		...
		std::cout<<"  -tdr <triangle depth range>"<<std::endl;
		std::cout<<"     Sets the initial triangle depth range of all subsequent 3D video sources"<<std::endl;
		std::cout<<"  -save <stream file name base>"<<std::endl;
		std::cout<<"     Saves 3D video streams from all connected sources"<<std::endl;
	...
	/* Save streams if requested: */
	if(saveFileName!=0)
		{
		startSavingStreams(saveFileName);
		
		/* Turn the "Save Streams..." toggle button on: */
		saveStreamsToggle->setToggle(true);
		}
	}

KinectViewer::~KinectViewer(void)
	{
	/* Stop streaming on all streamers: */
	for(std::vector<KinectStreamer*>::iterator sIt=streamers.begin();sIt!=streamers.end();++sIt)
		(*sIt)->stopStreaming();
	...
	/* Stop saving 3D video streams, if that's active: */
	for(size_t i=0;i<streamers.size();++i)
		streamers[i]->setFrameSaver(0);
	}
